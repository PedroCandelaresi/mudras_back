...
exports.SeedMudrasAuth1714848001000 = void 0;
const crypto_1 = require("crypto");
const bcrypt = require("bcrypt");
class SeedMudrasAuth1714848001000 {
    constructor() {
        this.name = 'SeedMudrasAuth1714848001000';
    }
    async up(queryRunner) {
        ...
        for (const r of roles) {
            ...
            roleIds[r.name] = id;
            await queryRunner.query(`INSERT INTO mudras_auth_roles (id, name, slug) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE name = VALUES(name)`, [id, r.name, r.slug]);
        }
        const permissions = [
        ...
        for (const p of permissions) {
            ...
            await queryRunner.query(`INSERT INTO mudras_auth_permissions (id, resource, action, description) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE description = VALUES(description)`, [id, p.resource, p.action, p.description ?? null]);
        }
        const assignAllToAdmin = Object.values(permissionIds);
        for (const permId of assignAllToAdmin) {
            await queryRunner.query(`INSERT IGNORE INTO mudras_auth_role_permissions (role_id, permission_id) VALUES (?, ?)`, [roleIds['ADMINISTRADOR'], permId]);
        }
        const allow = async (roleName, pairs) => {
            for (const [resource, action] of pairs) {
                const pid = permissionIds[`${resource}:${action}`];
                if (pid) {
                    await queryRunner.query(`INSERT IGNORE INTO mudras_auth_role_permissions (role_id, permission_id) VALUES (?, ?)`, [roleIds[roleName], pid]);
                }
            }
        };
        ...
        for (const u of users) {
            ...
            await queryRunner.query(`INSERT INTO mudras_auth_users (id, username, display_name, user_type, password_hash, must_change_password, is_active) 
         VALUES (?, ?, ?, 'EMPRESA', ?, 1, 1)
         ON DUPLICATE KEY UPDATE display_name = VALUES(display_name)`, [uid, u.username, u.display_name, passwordHash]);
            await queryRunner.query(`INSERT IGNORE INTO mudras_auth_user_roles (user_id, role_id) 
         SELECT id, ? FROM mudras_auth_users WHERE username = ?`, [roleIds[u.role], u.username]);
        }
    }
...
exports.SeedMudrasAuth1714848001000 = SeedMudrasAuth1714848001000;
//# sourceMappingURL=1714848001000-SeedMudrasAuth.js.map